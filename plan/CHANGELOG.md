...
## 2026-04-24 比較表・GitHub 表の標準化

### 変更内容

- `report-format-kri` の `## 8. 比較まとめ` で、比較可能な項目は Markdown テーブルを優先するルールを追加
- GitHub リポジトリを扱う場合、リンク、スター数、ライセンス、最終更新、概要をテーブル化するルールを追加
- 数値、コスト、精度、速度、日付、ベンチマークなど比較可能な値は表の列として整理する方針を追加
- `report-fact-check` に比較表と GitHub 表の検証観点を追加
- `workflows/research-report-kri.md` に比較メトリクス抽出とテーブル化の手順を追加
- `## 8. 比較まとめ` は原則テーブル中心とし、細かいトピック列挙は表の列に収める方針へ変更

## 2026-04-24 A4 要約の文章形式固定

### 変更内容

- `report-format-a4` の出力テンプレートから箇条書きマーカーを削除
- A4 要約は各セクションを文章段落で記載し、箇条書き・番号付きリスト・チェックリストを使わないルールを追加
- `research-report-kri` workflow に A4 要約の文章形式要件を追加
- `report-fact-check` に A4 要約が箇条書き化していないか確認する検証観点を追加
